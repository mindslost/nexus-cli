# Add Delete and Theme Persistence to Nexus CLI
...
## Key Files & Context
- `nexus_cli/main.py`: Update the `list` command handler to support the `delete` action returned by the TUI.
- `nexus_cli/tui.py`: 
...
- `nexus_cli/config.py` (New File): A simple utility to load and save JSON configuration from `~/.nexus_config.json`.

## Implementation Steps

### 1. Configuration Utility (`nexus_cli/config.py`)
- Create a `load_config` and `save_config` function.
- Default theme should be "dark" (or based on system preference).

### 2. Update TUI (`nexus_cli/tui.py`)
...
- **Theme Toggling**:
    ...
    - In `NexusListApp.__init__`, load the theme and set `self.theme`.

### 3. Update main.py
- In `list_notes`, add a handler for `elif action == "delete": delete(data)`.

## Verification & Testing
- Run `nexus list`, select a note, press `d`, confirm, and ensure the note is deleted.
- Run `nexus list`, press `t` to change the theme, exit, and re-run to verify the theme is remembered.
