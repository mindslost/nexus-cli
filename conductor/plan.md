...
## Objective
Enhance the existing `ost list` command to display a full-screen interactive Terminal User Interface (TUI) using `Textual`. The user will be able to navigate the hierarchical PARA tree of their notes using arrow keys and press `o` to open or `e` to edit the currently selected note.

## Key Files & Context
- `pyproject.toml`: Needs to be updated to include `textual` in the dependencies.
- `ostraca_cli/main.py`: The `list` command needs to be refactored to launch the Textual app instead of printing a Rich tree. The `edit` and `open_note` commands are already present and can be reused.
- `ostraca_cli/tui.py` (New File): A dedicated file to house the Textual application class to keep `main.py` clean.
...
### 2. Create the Textual Interface (`ostraca_cli/tui.py`)
- Import necessary Textual widgets (`App`, `ComposeResult`, `Tree`).
- Define a `OstracaListApp` that extends `App`.
...
### 3. Refactor `list` Command (`ostraca_cli/main.py`)
- In the `list_notes` function, replace the Rich `Tree` printing logic with launching the `OstracaListApp`.
- Pass the fetched `results` (from `get_filtered_notes`) into the `OstracaListApp` upon initialization.
- Await/capture the result of `OstracaListApp().run()`.
...
- Run `ost list` and ensure it launches a full-screen TUI instead of static text.
...
- Verify that filtering with `ost list --para Project` still works and only populates the selected notes into the TUI.